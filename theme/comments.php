    ...
    <h2>
        <?php
			...
			if ( '1' === $itrade_comment_count ) {
				...
				printf(
					/* translators: 1: title. */
					esc_html__( 'One comment on &ldquo;%1$s&rdquo;', 'itrade' ),
					get_the_title()
				);
				...
				printf(
					...
					esc_html( _nx( '%1$s comment on &ldquo;%2$s&rdquo;', '%1$s comments on &ldquo;%2$s&rdquo;', $itrade_comment_count, 'comments title', 'itrade' ) ),
					number_format_i18n( $itrade_comment_count ),
					get_the_title()
				);
				// phpcs:enable WordPress.Security.EscapeOutput.OutputNotEscaped
			}
